...
class InputSchema(BaseModel):
    ...
    Relies on an instance of ANSYS MAPDL, as in:
    `/f/Ansys/ANSYS Inc/v241/ansys/bin/winx64/ANSYS241.exe -grpc -port 50052`
    """
...
class OutputSchema(BaseModel):
    """Outputs for the tess_simp_compliance Tesseract."""
    ...
    strain_energy: Array[(None,), Float32] = Field(
        description="The element-wise strain energy of the solution."
    )
    sensitivity: Array[(None,), Float32] = Field(
        description="The deriviative of compliance with respect to each densitiy variable."
    )
...
class SIMPElasticity:
    ...
    This class encapsulates the full workflow for running a SIMP
    (Solid Isotropic Material with Penalization) topology optimization
    analysis using ANSYS MAPDL for finite element analysis.
    """
    ...
    def solve(self) -> OutputSchema:
        ...
        Returns:
            OutputSchema containing compliance, strain energy, and sensitivity
        """
    ...
    def _define_simp_materials(self) -> None:
        ...
        Uses the SIMP (Solid Isotropic Material with Penalization) method to
        scale material properties based on density values. Creates a unique
        ...
        # Calculate all material properties at once
        ...
        # Execute all commands at once using input_strings
    ...
    def _assign_materials_to_elements(self) -> None:
        ...
        # Build all EMODIF commands as a list for batch execution
    ...
    def _extract_strain_energy(self) -> None:
        ...
        Uses APDL commands to extract strain energy from element table and
        write to a text file, then reorders to match input density array indexing.

        The *VGET command retrieves data in element number order (sorted by element ID),
        so we must reorder the results to match the element_numbers array order.
        """
        logger.debug("Extracting strain energy data...")

        # Enter POST1 mode if not already there
        self.mapdl.post1()
        self.mapdl.set(1, 1)  # Load first load step, first substep
        ...
        # *VGET retrieves element table values into an array parameter
        self.mapdl.run(f"*DIM,strain_e,ARRAY,{self.n_elements},1")
        self.mapdl.run("*VGET,strain_e,ELEM,1,ETAB,SENE, , ,2")

        # Write strain energy to text file using non_interactive mode for file operations
        # this is much faster than using a get command
        ...
        # Read the data - this is in sorted element number order
        strain_energy_sorted = np.loadtxt("strain_energy.txt")

        # Create mapping from element numbers to their position in sorted order
        # The *VGET command returns data sorted by element ID
        sorted_element_numbers = np.sort(self.element_numbers)

        # Create inverse mapping: for each element in self.element_numbers,
        # find its position in the sorted array
        # This tells us where to find each element's data in strain_energy_sorted
        reorder_indices = np.searchsorted(sorted_element_numbers, self.element_numbers)

        # Reorder strain energy to match the order of self.element_numbers
        # (which matches the input density array order)
    ...
    def _create_pvmesh(self) -> None:
        ...
        Creates a PyVista grid containing the mesh geometry with density and
        Young's modulus values stored as cell data, and displacement constraints,
        applied forces, and nodal displacements stored as point data (node data).
        """
        ...
        logger.info("Exported results to mesh_density.vtk")

    def __convert_celldata_for_pv(self, elem_array: np.array) -> np.array:
        """Convert cell_data to follow pyvista cell ordering."""
...
def apply(inputs: InputSchema) -> OutputSchema:
    """Run the tess_simp_compliance Tesseract.

    Args:
        inputs: Input parameters for the SIMP elasticity analysis
