...
"""A Tesseract whose OutputSchema carries non-array fields alongside an array.

``backend`` and ``converged`` are written by the endpoint, read by the caller and
never differentiated. They are legal in an ``OutputSchema`` and for
``abstract_eval`` to return, so they reach ``apply_tesseract`` as plain leaves next
to real avals.
"""
