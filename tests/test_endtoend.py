...
def test_univariate_tesseract_vmap_vectorized(served_batched_tesseract, use_jit):
    ...
    def f(x, y):
        return apply_tesseract(batched_tess, inputs=dict(x=x, y=y), vectorized=True)[
            "result"
        ]
...
@pytest.mark.parametrize("use_jit", [True, False])
def test_vectorized_apply_without_vmap(served_batched_tesseract, use_jit):
    """Test that vectorized=True works correctly without vmap (singleton batch axis)."""
    batched_tess = Tesseract.from_url(served_batched_tesseract)
    ...
    _assert_pytree_isequal(result, expected)
