...
class TestNoopHttp:
    ...
    def setup_inputs(self, noop_tesseract_http, array_size):
        jax.clear_caches()
        self.tess = noop_tesseract_http
        self.inputs = {"data": create_test_array(array_size)}

    def test_noop_http_apply_eager(self, benchmark):
        """Benchmark eager apply (no JIT) via from_image."""
        benchmark(apply_tesseract, self.tess, self.inputs)

    def test_noop_http_apply_jit(self, benchmark):
